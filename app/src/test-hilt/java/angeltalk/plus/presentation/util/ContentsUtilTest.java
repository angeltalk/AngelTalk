...
import angeltalk.plus.presentation.shadow.ShadowThumbnailUtil;
...
@Config(shadows = ShadowThumbnailUtil.class)
public class ContentsUtilTest {
    ...
    public void givenFileNameAndBitMapExists_whenCallImageSave_thenSaveImage() throws Exception {
        String fileName = ContentsUtil.getImagePath(context);
    ...
    public void givenVideoFile_whenCallSaveVideoThumbnail_thenMakeVideoThumbnailImage() throws Exception {
        // given
        FileUtil.copyFile(new File(ContentsUtil.getContentFolder(context) + File.separator + "airplane.mp4"), new File(ContentsUtil.getTempFolder(context) + File.separator + "airplane.mp4"));

        // when
        ContentsUtil.saveVideoThumbnail(context, ContentsUtil.getTempFolder(context) + File.separator + "airplane.mp4");

        // then
        assertThat(new File(ContentsUtil.getTempFolder(context) + File.separator + "airplane.jpg").exists()).isTrue();
    }
    ...
    public void givenSharedPhotoCard_whenCallGetTempCardModel_thenMakeTempCardModel() throws Exception {
        ...
        String folderPath = ContentsUtil.getTempFolder(context) + File.separator;
    ...
    public void givenPhotoCardAndTempFiles_whenCallCopySharedFiles_thenCopy() throws Exception {
        // given
        String folderPath = ContentsUtil.getTempFolder(context);
        ...
        CardModel cardModel = CardModel.builder()
                ...
                .contentPath(ContentsUtil.getImagePath(context))
                .voicePath(ContentsUtil.getVoicePath(context))
                .build();

        // when
        ContentsUtil.copySharedFiles(context, cardModel, folderPath);
    ...
    public void givenVideoCardAndTempFiles_whenCallCopySharedFiles_thenCopy() throws Exception {
        ...
        String videoPath = ContentsUtil.getVideoPath(context);
        CardModel cardModel = CardModel.builder()
                .name("copyVideoCard")
