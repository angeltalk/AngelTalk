...
import angeltalk.plus.presentation.custom.CategorySelectDialog;
...
public class ShareCardActivityTest extends UITest {
    ...
    // NOT @BindValue: ApplicationManager is @Inject-constructor-provided, so Hilt binds
    // the real instance regardless of @BindValue. Instead, after Robolectric creates the
    // Activity we reflection-replace `applicationManager` on the subject.
    angeltalk.plus.presentation.manager.ApplicationManager applicationManager = Mockito.mock(angeltalk.plus.presentation.manager.ApplicationManager.class);
    ...
    public void givenCategorySelectDialogShowing_whenClickFirstCategoryAndClickSaveButton_thenSetCategoryModelAndCurrentCardIndexAndMoveToCardListActivity() throws Exception {
        ...
        // then
        verify(applicationManager).setCategoryModel(any(CategoryModel.class));
        verify(applicationManager).setCurrentCardIndex(anyInt());
        ShadowActivity shadowActivity = shadowOf(kakaotalk_subject);
